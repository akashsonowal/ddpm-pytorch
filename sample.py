...
class Sampler:
    ...
    def _sample_x0(
        self, xt: torch.Tensor, n_steps: int
    ):  # reverse diffusion to get x0 from xt
    ...
    def sample_animation(
        self, n_samples: int = 1, n_frames: int = 1000, create_video: bool = True
    ):
        xt = torch.randn(
            [n_samples, self.image_channels, self.image_size, self.image_size],
            device=self.device,
    ...
    def interpolate(
        self, x1: torch.Tensor, x2: torch.Tensor, lambda_: float = 0.01, t_: int = 100
    ):  # t_ is the final noised time stamp
        n_samples = x1.shape[0]
        t = torch.full((n_samples,), t_, device=self.device)
        xt = (1 - lambda_) * self.diffusion.q_sample(
            x1, t
        ) + lambda_ * self.diffusion.q_sample(
            x2, t
        )  # forward diffusion
        return self._sample_x0(xt, t_, n_steps=1000)  # reverse diffusion
...
def main(animate=False):
    ...
    sampler = Sampler(
        diffusion_model, image_channels=1, image_size=32, device=device
    )  # MNIST data has 1 channel
